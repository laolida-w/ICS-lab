...
void  eval(char *cmdline) 
{
    ...
    if((pid=fork())==0){/*Child runs user code*/
        sigprocmask(SIG_SETMASK,&prev_all,NULL);
        if(tok.builtins==BUILTIN_NOHUP){/* nohup:block SIGHUP*/
            sigset_t mask_hup;
         ...
         if(tok.outfile!=NULL){/*infile and out file*/
            fd1=open(tok.outfile,O_RDWR,0);
            dup2(fd1,STDOUT_FILENO);
        }
        if(tok.infile!=NULL){
            fd2=open(tok.infile,O_RDWR,0);
...
int parseline(const char *cmdline, struct cmdline_tokens *tok) 
{
    ...
    while (buf < endbuf) {
        ...
        if (*buf == '\'' || *buf == '\"') {/*【\'】= 【'】, 【\"】 = 【"】*/
            ...
            buf++;
            next = strchr (buf, *(buf-1));/*find the next quoted*/
        } 
        else {
            /* Find next delimiter */
...
void bgfg_cmd(char**argv){
    ...
    if((readjid&&jid==0)||(!readjid&&pid==0)){
    ...
    int _state = (!strcmp(argv[0],"bg")) + 1; 
    job->state = _state; //重新设置状态
    kill(-(job->pid),SIGCONT); //发送状态 重启
    if(_state == BG)
...
void kill_cmd(char**argv){
    ...
    struct job_t* job = NULL;
    int jid=0,pid=0;
    bool readjid=(argv[1][0] == '%');
    bool readgroup;
    if(readjid){
        if((readgroup=(argv[1][1]=='-')))
           jid = atoi(argv[1] +2);
        else
            jid = atoi(argv[1] +1);
    }
    ...
    if((jid==0&&readjid)||(pid==0&&!readjid)){
        printf("%s: argument must be a PID or %%jobid\n", argv[0]);
        return;
    }
    if(!readjid){
...
{ 
    ...
    while ((pid = waitpid(-1,  &statusp,  WUNTRACED|WNOHANG|WCONTINUED)) >0){ 
        /* Reap a zombie child */  
        struct job_t* job = getjobpid(job_list, pid); 
        sigprocmask(SIG_BLOCK, &mask_all, &prev_all);

        if(WIFEXITED(statusp))  { //normal exit
        deletejob(job_list,pid); /* Delete the child from the job list */
        }
       else if(WIFCONTINUED(statusp)){/* SIGCONT*/
         job->state= BG;
       }
        else if (WIFSIGNALED(statusp)){  //SIGINT;delete
            sio_put("Job [%d] (%d) terminated by signal %d\n", job->jid, job->pid, WTERMSIG(statusp));
            deletejob(job_list, pid);
    	}
        else if(WIFSTOPPED(statusp)){//stop by CTRL^Z
...
int addjob(struct job_t *job_list, pid_t pid, int state, char *cmdline) 
{
    //write(STDOUT_FILENO,"ADD CALLED\n",11);
    int i;
    if (pid < 1)
        return 0;
...
int deletejob(struct job_t *job_list, pid_t pid) 
{
    //write(STDOUT_FILENO,"DEL CALLED\n",11);
    int i;
